import { pgTable, uuid, varchar, text, timestamp, integer, jsonb, pgEnum, unique, uniqueIndex, index, decimal, date, } from "drizzle-orm/pg-core";
...
export const areaPincodes = pgTable("area_pincodes", {
    id: uuid("id").primaryKey().defaultRandom(),
...
export const centreAreaMap = pgTable("centre_area_map", {
    id: uuid("id").primaryKey().defaultRandom(),
    ...
    centreServiceAreaUnique: unique("centre_service_area_unique").on(table.centreId, table.serviceAreaId),
...
export const procurementWindows = pgTable("procurement_windows", {
    id: uuid("id").primaryKey().defaultRandom(),
    ...
    commodityYearIdx: index("procurement_windows_commodity_year_idx").on(table.commodity, table.year),
}));
